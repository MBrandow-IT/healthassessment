...
export default function Login({ path = '/' }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const login:Boolean = await handleLogin({ access_token, expires_in, refresh_token })
      if (login) {
        router.push(path)
      } else {
        // show error message
  ...
  return (
    <div className='h-[100vh]'>
      <div className={`absolute w-full h-full transition-transform duration-500 ${showLoginForm === true && "-translate-y-full"}`} onClick={() => setShowLoginForm(true)}>
